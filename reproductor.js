...
function dataSaverCap(){return window.NVDataSettings?.getEffectiveVideoCap?.()||null}
function applyHlsQualityCap(h){const cap=dataSaverCap();if(!cap)return;const apply=()=>{try{const levels=h.levels||[];if(!levels.length)return;let idx=-1;levels.forEach((lv,i)=>{const hgt=Number(lv.height||0);if(hgt&&hgt<=cap&&(idx<0||hgt>Number(levels[idx].height||0)))idx=i});if(idx<0){let best=0;levels.forEach((lv,i)=>{if(Number(lv.height||0)>Number(levels[best]?.height||0)&&Number(lv.height||0)<=cap*1.35)best=i});idx=best}h.autoLevelCapping=idx;h.startLevel=idx;h.nextLevel=idx;h.currentLevel=idx}catch(e){console.warn('[NETVISION] no se pudo limitar calidad',e)}};if(h.levels?.length)apply();else h.once?.(Hls.Events.MANIFEST_PARSED,apply)}
function updateDataBadge(){const cap=dataSaverCap(),box=NV.$('#dataSaverBadge');if(!box)return;if(cap){box.textContent=`Ahorro de datos · hasta ${cap}p`;box.classList.remove('hidden')}else box.classList.add('hidden')}
...
async function playSource(u){
 ...
 if(isHls(u)){
  ...
  return await new Promise(resolve=>{
   ...
   h.on(Hls.Events.MANIFEST_PARSED,()=>{applyHlsQualityCap(h);updateDataBadge();if(getSettings?.().autoplay!==false)v.play().catch(()=>{});finish(true)});
