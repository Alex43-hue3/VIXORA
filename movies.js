...
(function(){
 ...
 function render(items=C.items){
   ...
   items.forEach(item=>{const c=document.createElement('article');c.className='catalog-card';c.tabIndex=0;c.setAttribute('role','button');c.setAttribute('aria-label','Abrir '+(item.title||'contenido')); const poster=item.image||item.banner||placeholder('NETVISION');const genres=itemGenres(item).slice(0,2).join(' · ');c.innerHTML=`<div class="catalog-poster"><img src="${NV.esc(poster)}" alt="${NV.esc(item.title)}" loading="lazy"><span class="rating">★ ${NV.esc(item.rating||'—')}</span><button class="play-card" aria-label="Reproducir"><i class="fa-solid fa-play"></i></button></div><div class="catalog-info"><h3 title="${NV.esc(item.title)}">${NV.esc(item.title)}</h3><span>${NV.esc(item.year||'')} · ${NV.esc(genres|| (C.mode==='movies'?'Película':'Serie'))}</span></div>`;c.onclick=()=>NV.openMedia(item,C.mode);c.querySelector('.play-card').onclick=e=>{e.stopPropagation();NV.openMedia(item,C.mode)};c.onkeydown=e=>{if(e.key==='Enter'||e.key===' '){e.preventDefault();NV.openMedia(item,C.mode)}};g.appendChild(c)})
 }
