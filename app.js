/* =========================================================
   NETVISION - APP PRINCIPAL
   TV EN VIVO + M3U/M3U8 + HLS.JS
========================================================= */

const M3U_FILE = "./canales.m3u";

let channels = [];
let currentChannel = null;
let selectedCategory = null;
let hls = null;

let videoPlayer;
let playerPlaceholder;
let selectedChannelName;
let selectedChannelCategory;
let currentChannelLogo;
let tvCategories;
let selectedCategoryTitle;
let selectedCategoryCount;
let selectedCategoryChannels;


/* =========================================================
   INICIO
========================================================= */

document.addEventListener("DOMContentLoaded", () => {
    cacheElements();
    setupNavigation();
    setupSearch();
    setupProfile();
    setupModal();
    loadM3U();
});


/* =========================================================
   ELEMENTOS
========================================================= */

function cacheElements() {
    videoPlayer = document.getElementById("videoPlayer");
    playerPlaceholder = document.getElementById("playerPlaceholder");
    selectedChannelName = document.getElementById("selectedChannelName");
    selectedChannelCategory = document.getElementById("selectedChannelCategory");
    currentChannelLogo = document.getElementById("currentChannelLogo");
    tvCategories = document.getElementById("tvCategories");
    selectedCategoryTitle = document.getElementById("selectedCategoryTitle");
    selectedCategoryCount = document.getElementById("selectedCategoryCount");
    selectedCategoryChannels = document.getElementById("selectedCategoryChannels");
}


/* =========================================================
   NAVEGACIÓN
========================================================= */

function setupNavigation() {

    const pageButtons = document.querySelectorAll("[data-page]");
    const pages = document.querySelectorAll(".page");
    const navLinks = document.querySelectorAll(".nav-link");

    const mobileMenuBtn =
        document.getElementById("mobileMenuBtn");

    const mainNav =
        document.getElementById("mainNav");


    pageButtons.forEach(button => {

        button.addEventListener("click", () => {

            const page =
                button.dataset.page;

            if (page) {
                showPage(page);
            }

        });

    });


    navLinks.forEach(link => {

        link.addEventListener("click", () => {

            const page =
                link.dataset.page;

            if (page) {
                showPage(page);
            }

        });

    });


    if (mobileMenuBtn && mainNav) {

        mobileMenuBtn.addEventListener(
            "click",
            () => {

                mainNav.classList.toggle(
                    "open"
                );

            }
        );

    }


    function showPage(pageName) {

        /*
         * Si salimos de TV,
         * detener completamente el canal.
         */

        if (pageName !== "tv") {
            detenerTV();
        }


        pages.forEach(page => {

            page.classList.toggle(
                "active-page",
                page.id === `page-${pageName}`
            );

        });


        navLinks.forEach(link => {

            link.classList.toggle(
                "active",
                link.dataset.page === pageName
            );

        });


        if (mainNav) {
            mainNav.classList.remove("open");
        }


        window.scrollTo({
            top: 0,
            behavior: "smooth"
        });

    }

}


/* =========================================================
   BÚSQUEDA SUPERIOR
========================================================= */

function setupSearch() {

    const searchBtn =
        document.getElementById("searchBtn");

    const searchOverlay =
        document.getElementById("searchOverlay");

    const closeSearch =
        document.getElementById("closeSearch");

    const globalSearch =
        document.getElementById("globalSearch");


    /*
     * Si todavía existe el buscador
     * inferior de TV, eliminarlo.
     */

    const oldChannelSearch =
        document.getElementById("channelSearch");


    if (oldChannelSearch) {

        const container =
            oldChannelSearch.closest(
                ".search-container"
            );

        if (container) {
            container.remove();
        } else {
            oldChannelSearch.remove();
        }

    }


    /*
     * Abrir buscador superior
     */

    if (searchBtn && searchOverlay) {

        searchBtn.addEventListener(
            "click",
            () => {

                searchOverlay.classList.add(
                    "active"
                );


                if (globalSearch) {

                    globalSearch.value = "";

                    renderSearchResults(
                        [],
                        ""
                    );


                    setTimeout(
                        () => {
                            globalSearch.focus();
                        },
                        100
                    );

                }

            }
        );

    }


    /*
     * Cerrar con X
     */

    if (closeSearch && searchOverlay) {

        closeSearch.addEventListener(
            "click",
            () => {

                searchOverlay.classList.remove(
                    "active"
                );

            }
        );

    }


    /*
     * Cerrar haciendo click afuera
     */

    if (searchOverlay) {

        searchOverlay.addEventListener(
            "click",
            event => {

                if (
                    event.target ===
                    searchOverlay
                ) {

                    searchOverlay.classList.remove(
                        "active"
                    );

                }

            }
        );

    }


    /*
     * Buscar mientras escribes
     */

    if (globalSearch) {

        globalSearch.addEventListener(
            "input",
            () => {

                const term =
                    globalSearch.value
                        .trim()
                        .toLowerCase();


                if (!term) {

                    renderSearchResults(
                        [],
                        ""
                    );

                    return;

                }


                const results =
                    channels.filter(
                        channel => {

                            const name =
                                cleanChannelName(
                                    channel.name
                                ).toLowerCase();


                            const category =
                                String(
                                    channel.category ||
                                    ""
                                ).toLowerCase();


                            return (
                                name.includes(term) ||
                                category.includes(term)
                            );

                        }
                    );


                renderSearchResults(
                    results,
                    term
                );

            }
        );


        /*
         * ENTER
         */

        globalSearch.addEventListener(
            "keydown",
            event => {

                if (
                    event.key !== "Enter"
                ) {
                    return;
                }


                const term =
                    globalSearch.value
                        .trim()
                        .toLowerCase();


                if (!term) {
                    return;
                }


                const result =
                    channels.find(
                        channel => {

                            const name =
                                cleanChannelName(
                                    channel.name
                                ).toLowerCase();


                            const category =
                                String(
                                    channel.category ||
                                    ""
                                ).toLowerCase();


                            return (
                                name.includes(term) ||
                                category.includes(term)
                            );

                        }
                    );


                if (result) {

                    openChannelFromSearch(
                        result
                    );

                }

            }
        );

    }

}


/* =========================================================
   RESULTADOS DEL BUSCADOR
========================================================= */

function renderSearchResults(
    results,
    term = ""
) {

    const searchResults =
        document.getElementById(
            "searchResults"
        );


    /*
     * Si el HTML tiene un contenedor
     * #searchResults lo usamos.
     */

    if (searchResults) {

        searchResults.innerHTML = "";


        if (!term) {

            searchResults.innerHTML = `

                <div class="search-empty">

                    Escribe el nombre de
                    una película, serie o canal.

                </div>

            `;

            return;

        }


        if (!results.length) {

            searchResults.innerHTML = `

                <div class="search-empty">

                    🔎 No encontramos resultados
                    para "<strong>${escapeHTML(term)}</strong>".

                </div>

            `;

            return;

        }


        results.forEach(channel => {

            const item =
                document.createElement("button");


            item.type = "button";

            item.className =
                "search-result-item";


            item.innerHTML = `

                <span class="search-result-logo">

                    ${
                        channel.logo

                        ? `
                            <img
                                src="${escapeAttribute(
                                    channel.logo
                                )}"
                                alt="${escapeAttribute(
                                    channel.name
                                )}"
                                loading="lazy"
                                onerror="
                                    this.style.display='none';
                                "
                            >
                          `

                        : "📺"
                    }

                </span>


                <span class="search-result-info">

                    <strong>
                        ${escapeHTML(
                            cleanChannelName(
                                channel.name
                            )
                        )}
                    </strong>


                    <small>
                        ${escapeHTML(
                            channel.category ||
                            "Otros"
                        )}
                        · EN VIVO
                    </small>

                </span>


                <span class="search-result-arrow">
                    ›
                </span>

            `;


            item.addEventListener(
                "click",
                () => {

                    openChannelFromSearch(
                        channel
                    );

                }
            );


            searchResults.appendChild(
                item
            );

        });


        return;

    }


    /*
     * Compatibilidad con el diseño
     * anterior si no existe #searchResults.
     */

    const searchOverlay =
        document.getElementById(
            "searchOverlay"
        );


    if (
        !searchOverlay ||
        !term
    ) {
        return;
    }


    let fallback =
        searchOverlay.querySelector(
            ".search-results-fallback"
        );


    if (!fallback) {

        fallback =
            document.createElement(
                "div"
            );

        fallback.className =
            "search-results-fallback";

        searchOverlay.appendChild(
            fallback
        );

    }


    fallback.innerHTML = "";


    if (!results.length) {

        fallback.innerHTML = `

            <div class="search-empty">

                🔎 No encontramos resultados
                para "<strong>${escapeHTML(term)}</strong>".

            </div>

        `;

        return;

    }


    results.forEach(channel => {

        const item =
            document.createElement(
                "button"
            );


        item.type = "button";

        item.className =
            "search-result-item";


        item.innerHTML = `

            <span class="search-result-logo">

                ${
                    channel.logo

                    ? `
                        <img
                            src="${escapeAttribute(
                                channel.logo
                            )}"
                            alt="${escapeAttribute(
                                channel.name
                            )}"
                            loading="lazy"
                            onerror="
                                this.style.display='none';
                            "
                        >
                      `

                    : "📺"
                }

            </span>


            <span class="search-result-info">

                <strong>
                    ${escapeHTML(
                        cleanChannelName(
                            channel.name
                        )
                    )}
                </strong>


                <small>
                    ${escapeHTML(
                        channel.category ||
                        "Otros"
                    )}
                    · EN VIVO
                </small>

            </span>


            <span class="search-result-arrow">
                ›
            </span>

        `;


        item.addEventListener(
            "click",
            () => {

                openChannelFromSearch(
                    channel
                );

            }
        );


        fallback.appendChild(
            item
        );

    });

}


/* =========================================================
   ABRIR CANAL DESDE BUSCADOR
========================================================= */

function openChannelFromSearch(
    channel
) {

    if (!channel) {
        return;
    }


    /*
     * Cerrar buscador
     */

    const searchOverlay =
        document.getElementById(
            "searchOverlay"
        );


    if (searchOverlay) {

        searchOverlay.classList.remove(
            "active"
        );

    }


    /*
     * Mostrar TV
     */

    const pages =
        document.querySelectorAll(
            ".page"
        );


    const tvPage =
        document.getElementById(
            "page-tv"
        );


    pages.forEach(page => {

        page.classList.toggle(
            "active-page",
            page === tvPage
        );

    });


    /*
     * Activar TV
     */

    document
        .querySelectorAll(".nav-link")
        .forEach(link => {

            link.classList.toggle(
                "active",
                link.dataset.page === "tv"
            );

        });


    /*
     * Crear categorías
     */

    if (
        tvCategories &&
        channels.length
    ) {

        renderTV(channels);

    }


    /*
     * Seleccionar categoría
     */

    if (channel.category) {

        selectedCategory =
            channel.category;


        selectCategory(
            channel.category
        );

    }


    /*
     * Reproducir canal
     */

    selectChannel(
        channel
    );


    /*
     * Subir al reproductor
     */

    window.scrollTo({
        top: 0,
        behavior: "smooth"
    });

}


/* =========================================================
   PERFIL
========================================================= */

function setupProfile() {

    const profileBtn =
        document.getElementById(
            "profileBtn"
        );


    const profileMenu =
        document.getElementById(
            "profileMenu"
        );


    if (
        !profileBtn ||
        !profileMenu
    ) {
        return;
    }


    /*
     * Crear botón X si no existe
     */

    let closeBtn =
        profileMenu.querySelector(
            ".profile-close-btn"
        );


    const profileCard =
        profileMenu.querySelector(
            ".profile-card"
        ) ||
        profileMenu;


    if (!closeBtn) {

        closeBtn =
            document.createElement(
                "button"
            );


        closeBtn.type =
            "button";


        closeBtn.className =
            "profile-close-btn";


        closeBtn.setAttribute(
            "aria-label",
            "Cerrar perfil"
        );


        closeBtn.innerHTML =
            "×";


        if (profileCard) {

            profileCard.prepend(
                closeBtn
            );

        }

    }


    /*
     * Abrir
     */

    profileBtn.addEventListener(
        "click",
        event => {

            event.preventDefault();

            event.stopPropagation();

            profileMenu.classList.toggle(
                "active"
            );

        }
    );


    /*
     * Cerrar con X
     */

    closeBtn.addEventListener(
        "click",
        event => {

            event.preventDefault();

            event.stopPropagation();

            profileMenu.classList.remove(
                "active"
            );

        }
    );


    /*
     * Cerrar haciendo click afuera
     */

    document.addEventListener(
        "click",
        event => {

            if (
                !profileMenu.contains(
                    event.target
                ) &&
                event.target !== profileBtn
            ) {

                profileMenu.classList.remove(
                    "active"
                );

            }

        }
    );


    /*
     * ESC
     */

    document.addEventListener(
        "keydown",
        event => {

            if (
                event.key ===
                "Escape"
            ) {

                profileMenu.classList.remove(
                    "active"
                );

            }

        }
    );

}


/* =========================================================
   MODAL
========================================================= */

function setupModal() {

    const modal =
        document.getElementById(
            "contentModal"
        );


    const close =
        document.getElementById(
            "closeContent"
        );


    if (
        close &&
        modal
    ) {

        close.addEventListener(
            "click",
            () => {

                modal.classList.remove(
                    "active"
                );

            }
        );

    }


    if (modal) {

        modal.addEventListener(
            "click",
            event => {

                if (
                    event.target ===
                    modal
                ) {

                    modal.classList.remove(
                        "active"
                    );

                }

            }
        );

    }

}


/* =========================================================
   CARGAR M3U
========================================================= */

async function loadM3U() {

    try {

        console.log(
            "NETVISION: cargando",
            M3U_FILE
        );


        const response =
            await fetch(
                M3U_FILE,
                {
                    cache:
                        "no-store"
                }
            );


        if (!response.ok) {

            throw new Error(
                `HTTP ${response.status}`
            );

        }


        const text =
            await response.text();


        channels =
            parseM3U(text);


        console.log(
            "NETVISION: canales encontrados:",
            channels.length
        );


        if (!channels.length) {

            showError(
                "La lista M3U está vacía."
            );

            return;

        }


        renderTV(
            channels
        );


        renderHomeChannels();


    } catch (error) {

        console.error(
            "NETVISION: error cargando M3U",
            error
        );


        showError(
            "No se pudo cargar la lista. Verifica que exista canales.m3u."
        );

    }

}


/* =========================================================
   PARSER M3U
========================================================= */

function parseM3U(text) {

    const lines =
        text
            .split(/\r?\n/)
            .map(line =>
                line.trim()
            )
            .filter(Boolean);


    const result = [];


    for (
        let i = 0;
        i < lines.length;
        i++
    ) {

        const line =
            lines[i];


        if (
            !line.startsWith(
                "#EXTINF:"
            )
        ) {
            continue;
        }


        const info =
            line.substring(
                8
            );


        const commaIndex =
            info.indexOf(",");


        let attributes =
            info;


        let channelName =
            "Canal sin nombre";


        if (
            commaIndex !== -1
        ) {

            attributes =
                info.substring(
                    0,
                    commaIndex
                );


            channelName =
                info.substring(
                    commaIndex + 1
                ).trim() ||
                channelName;

        }


        let url = "";


        for (
            let j = i + 1;
            j < lines.length;
            j++
        ) {

            if (
                !lines[j].startsWith("#")
            ) {

                url =
                    lines[j];

                break;

            }

        }


        if (!url) {
            continue;
        }


        result.push({

            id:
                getAttribute(
                    attributes,
                    "tvg-id"
                ) ||
                createId(
                    channelName
                ),


            name:
                channelName,


            url:
                url,


            logo:
                getAttribute(
                    attributes,
                    "tvg-logo"
                ),


            category:
                getAttribute(
                    attributes,
                    "group-title"
                ) ||
                "Otros"

        });

    }


    return result;

}


function getAttribute(
    text,
    attribute
) {

    const regex =
        new RegExp(
            `${attribute}="([^"]*)"`,
            "i"
        );


    const match =
        text.match(
            regex
        );


    return match
        ? match[1]
        : "";

}


function createId(text) {

    return text
        .toLowerCase()
        .normalize("NFD")
        .replace(
            /[\u0300-\u036f]/g,
            ""
        )
        .replace(
            /[^a-z0-9]+/g,
            "-"
        )
        .replace(
            /^-+|-+$/g,
            "");

}


/* =========================================================
   TV
========================================================= */

function renderTV(
    list = channels
) {

    if (!tvCategories) {
        return;
    }


    tvCategories.innerHTML =
        "";


    const categories = [];


    list.forEach(
        channel => {

            if (
                !categories.includes(
                    channel.category
                )
            ) {

                categories.push(
                    channel.category
                );

            }

        }
    );


    categories.forEach(
        category => {

            const button =
                document.createElement(
                    "button"
                );


            button.type =
                "button";


            button.className =
                "tv-category-button";


            button.dataset.category =
                category;


            button.innerHTML = `

                <span class="category-icon">

                    ${getCategoryIcon(
                        category
                    )}

                </span>


                <span class="category-name">

                    ${escapeHTML(
                        category
                    )}

                </span>


                <span class="category-arrow">

                    ›

                </span>

            `;


            button.addEventListener(
                "click",
                () => {

                    selectCategory(
                        category
                    );

                }
            );


            tvCategories.appendChild(
                button
            );

        }
    );


    if (
        categories.length
    ) {

        const category =
            selectedCategory &&
            categories.includes(
                selectedCategory
            )
                ? selectedCategory
                : categories[0];


        selectCategory(
            category
        );

    }

}


/* =========================================================
   SELECCIONAR CATEGORÍA
========================================================= */

function selectCategory(
    category
) {

    selectedCategory =
        category;


    document
        .querySelectorAll(
            ".tv-category-button"
        )
        .forEach(
            button => {

                button.classList.toggle(
                    "active",
                    button.dataset.category ===
                    category
                );

            }
        );


    const categoryChannels =
        channels.filter(
            channel =>
                channel.category ===
                category
        );


    renderSelectedCategory(
        category,
        categoryChannels
    );

}


/* =========================================================
   MOSTRAR CANALES
========================================================= */

function renderSelectedCategory(
    category,
    categoryChannels
) {

    if (
        !selectedCategoryChannels
    ) {
        return;
    }


    if (
        selectedCategoryTitle
    ) {

        selectedCategoryTitle.innerHTML = `

            ${getCategoryIcon(
                category
            )}

            CANALES DE:

            <span>
                ${escapeHTML(
                    category
                )}
            </span>

        `;

    }


    if (
        selectedCategoryCount
    ) {

        selectedCategoryCount.textContent =
            `${categoryChannels.length} ${
                categoryChannels.length === 1
                    ? "canal"
                    : "canales"
            }`;

    }


    selectedCategoryChannels.innerHTML =
        "";


    categoryChannels.forEach(
        channel => {

            selectedCategoryChannels.appendChild(
                createChannelCard(
                    channel
                )
            );

        }
    );

}


/* =========================================================
   TARJETA DE CANAL
========================================================= */

function createChannelCard(
    channel
) {

    const card =
        document.createElement(
            "article"
        );


    card.className =
        "channel-card";


    card.tabIndex =
        0;


    const logo =
        channel.logo

            ? `

                <img
                    src="${escapeAttribute(
                        channel.logo
                    )}"
                    alt="${escapeAttribute(
                        channel.name
                    )}"
                    loading="lazy"
                    onerror="
                        this.style.display='none';
                    "
                >

              `

            : `

                <span class="default-channel-logo">
                    TV
                </span>

              `;


    card.innerHTML = `

        <div class="channel-logo">

            ${logo}

        </div>


        <div class="channel-card-info">

            <div class="channel-name">

                ${escapeHTML(
                    cleanChannelName(
                        channel.name
                    )
                )}

            </div>


            <div class="channel-meta">

                ● EN VIVO

            </div>

        </div>

    `;


    card.addEventListener(
        "click",
        () => {

            selectChannel(
                channel
            );

        }
    );


    card.addEventListener(
        "keydown",
        event => {

            if (
                event.key ===
                    "Enter" ||
                event.key ===
                    " "
            ) {

                event.preventDefault();

                selectChannel(
                    channel
                );

            }

        }
    );


    return card;

}


/* =========================================================
   SELECCIONAR CANAL
========================================================= */

function selectChannel(
    channel
) {

    if (!channel) {
        return;
    }


    currentChannel =
        channel;


    console.log(
        "NETVISION: seleccionando",
        channel.name
    );


    if (
        selectedChannelName
    ) {

        selectedChannelName.textContent =
            cleanChannelName(
                channel.name
            );

    }


    if (
        selectedChannelCategory
    ) {

        selectedChannelCategory.textContent =
            `${channel.category} · EN VIVO`;

    }


    if (
        currentChannelLogo
    ) {

        currentChannelLogo.innerHTML =

            channel.logo

                ? `

                    <img
                        src="${escapeAttribute(
                            channel.logo
                        )}"
                        alt="${escapeAttribute(
                            channel.name
                        )}"
                    >

                  `

                : `<span>TV</span>`;

    }


    if (
        playerPlaceholder
    ) {

        playerPlaceholder.classList.add(
            "hidden"
        );

    }


    playStream(
        channel.url
    );

}


/* =========================================================
   REPRODUCIR HLS
========================================================= */

function playStream(
    url
) {

    if (!videoPlayer) {

        console.error(
            "NETVISION: no existe #videoPlayer"
        );

        return;

    }


    console.log(
        "NETVISION: reproduciendo",
        url
    );


    /*
     * Destruir reproducción anterior
     */

    if (hls) {

        try {
            hls.destroy();
        } catch (error) {}

        hls = null;

    }


    videoPlayer.pause();

    videoPlayer.removeAttribute(
        "src"
    );

    videoPlayer.load();


    /*
     * HLS.JS
     */

    if (
        window.Hls &&
        Hls.isSupported()
    ) {

        hls =
            new Hls({

                enableWorker:
                    true,

                lowLatencyMode:
                    true,

                backBufferLength:
                    90

            });


        hls.loadSource(
            url
        );


        hls.attachMedia(
            videoPlayer
        );


        hls.on(
            Hls.Events.MANIFEST_PARSED,
            () => {

                videoPlayer
                    .play()
                    .catch(
                        error => {

                            console.log(
                                "NETVISION: autoplay bloqueado",
                                error
                            );

                        }
                    );

            }
        );


        hls.on(
            Hls.Events.ERROR,
            (
                event,
                data
            ) => {

                console.error(
                    "NETVISION HLS ERROR:",
                    data
                );


                if (
                    !data.fatal
                ) {
                    return;
                }


                if (
                    data.type ===
                    Hls.ErrorTypes.NETWORK_ERROR
                ) {

                    hls.startLoad();


                } else if (
                    data.type ===
                    Hls.ErrorTypes.MEDIA_ERROR
                ) {

                    hls.recoverMediaError();


                } else {

                    showPlayerError();


                    try {
                        hls.destroy();
                    } catch (error) {}


                    hls = null;

                }

            }
        );


        return;

    }


    /*
     * Safari / navegador con HLS nativo
     */

    if (
        videoPlayer.canPlayType(
            "application/vnd.apple.mpegurl"
        )
    ) {

        videoPlayer.src =
            url;


        videoPlayer.addEventListener(
            "loadedmetadata",
            () => {

                videoPlayer
                    .play()
                    .catch(
                        error => {

                            console.log(
                                "NETVISION: autoplay bloqueado",
                                error
                            );

                        }
                    );

            },
            {
                once: true
            }
        );


        return;

    }


    showPlayerError();

}


/* =========================================================
   DETENER TV
========================================================= */

function detenerTV() {

    currentChannel =
        null;


    /*
     * Destruir HLS
     */

    if (hls) {

        try {
            hls.stopLoad();
        } catch (error) {}


        try {
            hls.detachMedia();
        } catch (error) {}


        try {
            hls.destroy();
        } catch (error) {}


        hls =
            null;

    }


    /*
     * Detener video
     */

    if (videoPlayer) {

        try {
            videoPlayer.pause();
        } catch (error) {}


        videoPlayer.removeAttribute(
            "src"
        );


        videoPlayer.src =
            "";


        try {
            videoPlayer.load();
        } catch (error) {}

    }


    /*
     * Limpiar información
     */

    if (
        selectedChannelName
    ) {

        selectedChannelName.textContent =
            "Ningún canal seleccionado";

    }


    if (
        selectedChannelCategory
    ) {

        selectedChannelCategory.textContent =
            "Selecciona un canal";

    }


    if (
        currentChannelLogo
    ) {

        currentChannelLogo.innerHTML =
            "<span>TV</span>";

    }


    /*
     * Mostrar placeholder
     */

    if (
        playerPlaceholder
    ) {

        playerPlaceholder.classList.remove(
            "hidden"
        );


        playerPlaceholder.innerHTML = `

            <div class="player-icon">

                ▶

            </div>


            <h3>

                Selecciona un canal

            </h3>


            <p>

                Elige un canal para comenzar
                a reproducir.

            </p>

        `;

    }

}


/* =========================================================
   ERROR DEL REPRODUCTOR
========================================================= */

function showPlayerError() {

    if (
        !playerPlaceholder
    ) {
        return;
    }


    playerPlaceholder.classList.remove(
        "hidden"
    );


    playerPlaceholder.innerHTML = `

        <div class="player-icon">

            ⚠

        </div>


        <h3>

            No se pudo reproducir

        </h3>


        <p>

            El canal no está disponible
            en este momento.

        </p>

    `;

}


/* =========================================================
   CANALES DE INICIO
========================================================= */

function renderHomeChannels() {

    const popular =
        document.getElementById(
            "homePopularChannels"
        );


    const newest =
        document.getElementById(
            "homeNewChannels"
        );


    /*
     * MÁS VISTOS
     */

    if (popular) {

        popular.innerHTML =
            "";


        channels
            .slice(
                0,
                6
            )
            .forEach(
                channel => {

                    const card =
                        createHomeChannelCard(
                            channel
                        );


                    popular.appendChild(
                        card
                    );

                }
            );

    }


    /*
     * NUEVOS
     */

    if (newest) {

        newest.innerHTML =
            "";


        channels
            .slice(
                -6
            )
            .reverse()
            .forEach(
                channel => {

                    const card =
                        createHomeChannelCard(
                            channel
                        );


                    newest.appendChild(
                        card
                    );

                }
            );

    }

}


/*
 * Tarjeta especial para Inicio.
 * Al hacer clic abre TV y reproduce.
 */

function createHomeChannelCard(
    channel
) {

    const card =
        createChannelCard(
            channel
        );


    /*
     * El click original de
     * createChannelCard se mantiene,
     * por eso aquí reemplazamos
     * el comportamiento clonando
     * el elemento.
     */

    const newCard =
        card.cloneNode(
            true
        );


    newCard.replaceWith(
        newCard
    );


    newCard.addEventListener(
        "click",
        event => {

            event.stopPropagation();

            openChannelFromSearch(
                channel
            );

        }
    );


    newCard.addEventListener(
        "keydown",
        event => {

            if (
                event.key ===
                    "Enter" ||
                event.key ===
                    " "
            ) {

                event.preventDefault();

                event.stopPropagation();

                openChannelFromSearch(
                    channel
                );

            }

        }
    );


    return newCard;

}


/* =========================================================
   ICONOS DE CATEGORÍA
========================================================= */

function getCategoryIcon(
    category
) {

    const name =
        category
            .toLowerCase()
            .normalize("NFD")
            .replace(
                /[\u0300-\u036f]/g,
                ""
            );


    if (
        name.includes(
            "relig"
        )
    ) {
        return "✝️";
    }


    if (
        name.includes(
            "anime"
        )
    ) {
        return "🍥";
    }


    if (
        name.includes(
            "caricatura"
        ) ||
        name.includes(
            "kids"
        ) ||
        name.includes(
            "infantil"
        )
    ) {

        return "🎨";

    }


    if (
        name.includes(
            "deporte"
        ) ||
        name.includes(
            "sport"
        )
    ) {

        return "⚽";

    }


    if (
        name.includes(
            "curiosity"
        ) ||
        name.includes(
            "document"
        )
    ) {

        return "🔬";

    }


    if (
        name.includes(
            "noticia"
        ) ||
        name.includes(
            "news"
        )
    ) {

        return "📰";

    }


    if (
        name.includes(
            "musica"
        ) ||
        name.includes(
            "music"
        )
    ) {

        return "🎵";

    }


    if (
        name.includes(
            "pelicula"
        ) ||
        name.includes(
            "movie"
        )
    ) {

        return "🎬";

    }


    return "📺";

}


/* =========================================================
   LIMPIAR NOMBRE
========================================================= */

function cleanChannelName(
    name
) {

    if (!name) {
        return "Canal";
    }


    return name
        .replace(
            /\s*\[[^\]]*\]\s*/g,
            " "
        )
        .replace(
            /\s+/g,
            " "
        )
        .trim();

}


/* =========================================================
   SEGURIDAD HTML
========================================================= */

function escapeHTML(
    value
) {

    return String(
        value ?? ""
    )
        .replace(
            /&/g,
            "&amp;"
        )
        .replace(
            /</g,
            "&lt;"
        )
        .replace(
            />/g,
            "&gt;"
        )
        .replace(
            /"/g,
            "&quot;"
        )
        .replace(
            /'/g,
            "&#039;"
        );

}


function escapeAttribute(
    value
) {

    return escapeHTML(
        value
    );

}


/* =========================================================
   ERROR GENERAL
========================================================= */

function showError(
    message
) {

    console.error(
        "NETVISION:",
        message
    );


    if (
        tvCategories
    ) {

        tvCategories.innerHTML = `

            <div
                style="
                    padding:15px;
                    color:#ff6b6b;
                    font-size:13px;
                "
            >

                ⚠️

                ${escapeHTML(
                    message
                )}

            </div>

        `;

    }

}


/* =========================================================
   LIMPIEZA AL SALIR
========================================================= */

window.addEventListener(
    "beforeunload",
    () => {

        detenerTV();

    }
);
