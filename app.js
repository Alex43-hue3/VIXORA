...
NV.closeAllModals=()=>{NV.$$('.modal').forEach(m=>m.classList.add('hidden'))};
NV.openMyList=()=>{NV.closeAllModals();NV.$('#myListModal')?.classList.remove('hidden');NV.renderMyList?.()};
NV.renderMyList=()=>{
...
function bind(){
 NV.$('#tvConnectBtn')?.addEventListener('click',()=>{location.href=location.pathname+'?connect=1'});
