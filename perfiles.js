...
function getSettings(){try{return Object.assign({autoplay:true,reducedMotion:false,dataSaver:false,quality:'auto'},JSON.parse(localStorage.getItem(SETTINGS_KEY)||'{}'))}catch{return {autoplay:true,reducedMotion:false,dataSaver:false,quality:'auto'}}}
function deviceIsMobile(){return /Android|iPhone|iPad|iPod|Mobile/i.test(navigator.userAgent||'')}
function connectionInfo(){return navigator.connection||navigator.mozConnection||navigator.webkitConnection||null}
function recommendedMobileQuality(){const c=connectionInfo();if(c?.saveData)return 360;const e=String(c?.effectiveType||'').toLowerCase();if(e==='slow-2g'||e==='2g')return 360;if(e==='3g')return 480;return 480}
function getEffectiveVideoCap(){const s=getSettings();const q=String(s.quality||'auto');if(q!=='auto')return Number(q);if(s.dataSaver&&deviceIsMobile())return recommendedMobileQuality();if(deviceIsMobile()){const c=connectionInfo();if(c?.saveData)return 360;return null}return null}
window.NVDataSettings={getSettings,saveSettings,getEffectiveVideoCap,deviceIsMobile,connectionInfo,recommendedMobileQuality};
...
NV.openSettings=()=>{const s=getSettings();const q=String(s.quality||'auto');NV.$('#settingAutoplay').checked=!!s.autoplay;NV.$('#settingReducedMotion').checked=!!s.reducedMotion;NV.$('#settingDataSaver').checked=!!s.dataSaver;NV.$('#settingQuality').value=['auto','1080','720','480','360'].includes(q)?q:'auto';updateDataSettingUI();NV.$('#settingsModal').classList.remove('hidden')}
function updateDataSettingUI(){const s=getSettings();const q=String(s.quality||'auto');const hint=NV.$('#settingQualityHint');const status=NV.$('#dataSaverStatus');if(hint){if(q==='auto')hint.textContent=s.dataSaver?'En móvil, NETVISION limitará la calidad según tu conexión.':'Automática: NETVISION elige la mejor calidad disponible.';else hint.textContent=`Límite máximo: ${q}p. Una fuente puede ofrecer menos.`}if(status){const cap=getEffectiveVideoCap();if(s.dataSaver&&deviceIsMobile())status.textContent=`Ahorro activo · límite recomendado ${cap||480}p`;else if(s.dataSaver)status.textContent='Ahorro activo · se aplicará al usar un dispositivo móvil';else status.textContent='Ahorro desactivado';status.className='data-saver-status '+(s.dataSaver?'active':'')}}
...
NV.$('#settingDataSaver').onchange=e=>{const s=getSettings();s.dataSaver=e.target.checked;saveSettings(s);updateDataSettingUI();NV.toast(s.dataSaver?'Ahorro de datos activado':'Ahorro de datos desactivado')};
NV.$('#settingQuality').onchange=e=>{const s=getSettings();s.quality=e.target.value;saveSettings(s);updateDataSettingUI();NV.toast(`Calidad máxima: ${e.target.value==='auto'?'automática':e.target.value+'p'}`)};
NV.$('#settingsResetPreferences').onclick=()=>{if(!confirm('¿Restablecer las preferencias de NETVISION?'))return;saveSettings({autoplay:true,reducedMotion:false,dataSaver:false,quality:'auto'});NV.openSettings();NV.toast('Preferencias restablecidas')};
NV.$$('#settingsModal [data-close-settings]').forEach(b=>b.onclick=closeSettings);
saveSettings(getSettings());updateDataSettingUI();
