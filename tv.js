...
NV.$('#channelSearch').oninput=renderChannels;NV.$('#openSidebar').onclick=()=>NV.$('#channelSidebar').classList.add('open');NV.$('#closeSidebar').onclick=()=>NV.$('#channelSidebar').classList.remove('open');NV.$('#tvPrev').onclick=()=>NV.changeChannel(-1);NV.$('#tvNext').onclick=()=>NV.changeChannel(1);NV.$('#fsPrev').onclick=()=>NV.changeChannel(-1);NV.$('#fsNext').onclick=()=>NV.changeChannel(1);NV.$('#fsClose').onclick=()=>document.exitFullscreen?.();NV.$('#tvVolDown').onclick=()=>{const v=NV.$('#tvVideo');v.volume=Math.max(0,v.volume-.1)};NV.$('#tvMute').onclick=()=>{const v=NV.$('#tvVideo');v.muted=!v.muted};NV.$('#tvVolUp').onclick=()=>{const v=NV.$('#tvVideo');v.muted=false;v.volume=Math.min(1,v.volume+.1)};NV.$('#tvFullscreen').onclick=async()=>{const p=NV.$('#tvPlayer');if(!document.fullscreenElement&&!p.classList.contains('remote-fullscreen')){try{await p.requestFullscreen({navigationUI:'hide'})}catch{p.classList.add('remote-fullscreen');document.body.classList.add('nv-remote-fullscreen')}}else if(document.fullscreenElement)document.exitFullscreen?.();else{p.classList.remove('remote-fullscreen');document.body.classList.remove('nv-remote-fullscreen')}};document.addEventListener('fullscreenchange',()=>{const p=NV.$('#tvPlayer');if(!p)return;p.classList.toggle('is-fullscreen',document.fullscreenElement===p)});
