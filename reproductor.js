...
async function playSource(u){
 ...
 if(isHls(u)){
  ...
  return await new Promise(resolve=>{
   ...
   h.on(Hls.Events.MANIFEST_PARSED,()=>{if(getSettings?.().autoplay!==false){v.muted=!window.__NVTV_UNLOCKED;v.play().catch(()=>{})}finish(true)});
...
async function nativeVideo(u){
 ...
 try{await waitVideo(v,12000);if(getSettings?.().autoplay!==false){v.muted=!window.__NVTV_UNLOCKED;v.play().catch(()=>{})}return true}catch{v.pause();v.removeAttribute('src');v.load();return false}
}
...
NV.$('#mediaVolDown').onclick=()=>{const v=NV.$('#mediaVideo');v.volume=Math.max(0,v.volume-.1);v.muted=false};NV.$('#mediaMute').onclick=()=>{const v=NV.$('#mediaVideo');v.muted=!v.muted};NV.$('#mediaVolUp').onclick=()=>{const v=NV.$('#mediaVideo');v.muted=false;v.volume=Math.min(1,v.volume+.1)};NV.$('#mediaFullscreen').onclick=async()=>{const p=NV.$('#mediaPlayer');if(!document.fullscreenElement&&!p.classList.contains('remote-fullscreen')){try{await p.requestFullscreen({navigationUI:'hide'})}catch{p.classList.add('is-fullscreen')}}else if(document.fullscreenElement)document.exitFullscreen?.();else p.classList.remove('remote-fullscreen');};
