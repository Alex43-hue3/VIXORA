...
'use strict';
(function(){
 const CACHE_KEY='netvision_catalog_cache_v2';
 ...
 async function prefetchAll(){
   ...
   const batchSize=C.mode==='movies'?8:8;
   C.prefetchPromise=(async()=>{
     try{
       ...
       while(true){
         ...
         const results=await Promise.all(pages.map(page=>NVApi.list(C.mode,page).catch(e=>({items:[],hasNext:false,totalPages:0,page,errors:[e.message]}))));
         let got=0;
         ...
         if(!C.searching && (loadedPages===pages.length || loadedPages%24===0)){
           renderGenres();
           if(C.genre==='Todos'&&!C.query)renderItems();
         }
         if(loadedPages%24===0 || !C.hasNext){
           writeCache(C.mode,{savedAt:Date.now(),items:C.items,page:C.page,hasNext:C.hasNext,totalPages:C.totalPages,genreList:C.genreList});
         }

         const elapsed=((performance.now()-started)/1000).toFixed(1);
         const progress=knownTotal?`Página ${Math.min(C.page,knownTotal)}/${knownTotal}`:`Páginas procesadas: ${loadedPages}`;
         setText('#catalogStatus',`Construyendo catálogo doble… ${C.items.length} títulos · ${progress} · ${elapsed}s`);

         if(!got && !anyProviderNext) break;
         if(knownTotal && C.page>=knownTotal) break;
         if(!knownTotal && noContentStreak>=2) break;
         next=C.page+1;
       }
       ...
       const elapsed=((performance.now()-started)/1000).toFixed(1);
       writeCache(C.mode,{savedAt:Date.now(),items:C.items,page:C.page,hasNext:C.hasNext,totalPages:C.totalPages,genreList:C.genreList,complete:actuallyComplete,providers:'double'});
