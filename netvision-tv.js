...
(function(){
    ...
    if(mode==='connect'){
      ...
      d.innerHTML=`<div class="remote11-wrap">
        ...
        <div id="remote11Quick" class="remote11-quick hidden">
          <button data-cmd="playpause">▶︎ / ⏸</button><button data-cmd="prev">⏮</button><button data-cmd="next">⏭</button><button data-cmd="fullscreen">⛶ Pantalla</button>
          <button data-cmd="voldown">🔉 Volumen −</button><button data-cmd="volup">🔊 Volumen +</button>
          <button data-cmd="mute">🔇 Silenciar</button><button data-cmd="home">⌂ Menú</button>
          <button data-cmd="disconnect">Desconectar</button>
        </div>
        <div class="remote11-tip">El teléfono controla la interfaz completa de NETVISION en la TV. Incluye navegación, reproducción, volumen y pantalla completa.</div>
      </div>`;
    }
  ...
  function getFocusables(){
    ...
    const selectors='button:not([disabled]),input:not([disabled]),[tabindex="0"],.catalog-card,.channel-item,.genre-chip,.home-card';
    return [...screen.querySelectorAll(selectors)].filter(e=>{
      if(e.closest('.catalog-card') && e.closest('.catalog-card')!==e)return false;
  ...
  function getVideo(){
    const media=$('#mediaVideo'),tv=$('#tvVideo');
    if(NV?.state?.view==='media'&&media)return media;
    if(NV?.state?.view==='tv'&&tv)return tv;
    return media||tv||null;
  }
  function togglePseudoFullscreen(){
    const target=NV?.state?.view==='media'?$('#mediaPlayer'):$('#tvPlayer');
    if(!target)return;
    const active=target.classList.toggle('remote-fullscreen');
    document.body.classList.toggle('nv-remote-fullscreen',active);
    const b=$('#mediaFullscreen');if(b&&NV?.state?.view==='media')b.innerHTML=active?'✕':'<i class="fa-solid fa-expand"></i>';
  }
  function setVolume(delta){const v=getVideo();if(!v)return;v.muted=false;v.volume=Math.max(0,Math.min(1,(v.volume||0)+delta));NV?.toast?.('Volumen '+Math.round(v.volume*100)+'%');}
  function toggleMute(){const v=getVideo();if(!v)return;v.muted=!v.muted;NV?.toast?.(v.muted?'Silenciado':'Sonido activado');}
  function tvCommand(c){
    ...
    if(c==='back'){if(document.activeElement?.tagName==='INPUT'){document.activeElement.blur();return}if(document.fullscreenElement){document.exitFullscreen?.();return}const fs=document.querySelector('.remote-fullscreen');if(fs){fs.classList.remove('remote-fullscreen');document.body.classList.remove('nv-remote-fullscreen');return}if(NV?.state?.view!=='home')NV.setView('home');else document.querySelector('.modal:not(.hidden) [data-close],.modal:not(.hidden) [data-close-settings],.modal:not(.hidden) [data-close-switch]')?.click();return}
    if(c==='home'||c==='menu'){NV?.setView('home');return}
    if(c==='search'){const i=NV?.state?.view==='tv'?$('#channelSearch'):$('#catalogSearch');if(i){i.focus();i.select?.()}return}
    if(c==='prev'){NV?.changeChannel?.(-1);return}if(c==='next'){NV?.changeChannel?.(1);return}
    if(c==='fullscreen'){togglePseudoFullscreen();return}
    if(c==='playpause'){const v=getVideo();if(v){if(v.paused){const p=v.play();if(p?.catch)p.catch(()=>NV?.toast?.('La TV bloqueó la reproducción automática. Pulsa Reproducir una vez en la TV.'))}else v.pause()}return}
  ...
  function bindTV(){
    ...
    state.peer.on('connection',conn=>{
      ...
      conn.on('open',()=>{
        state.connected=true;status('✓ Teléfono conectado');state.focus=0;paintFocus();send(statePacket());hideTVPanel();
      });
      conn.on('data',d=>{
