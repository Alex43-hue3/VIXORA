/* NETVISION API CORE v5 - PROVEEDOR ÚNICO
   Proveedor seleccionado para pruebas: PelisPlusHD.
*/
'use strict';
...
(function(){
    ...
    for(let attempt=0; attempt<A.retries; attempt++){
      ...
      try{
        ...
        if(attempt<A.retries-1){
          const delay=350*Math.pow(2,attempt)+Math.round(Math.random()*250);
  ...
  // Comparte solicitudes por página. Si la carga completa y una búsqueda
  // necesitan la misma página al mismo tiempo, solo se hace una petición.
  ...
  async function list(kind,page=1){
    const key=`${kind}:${page}:pelisplushd`;
    if(pageRequests.has(key)) return pageRequests.get(key);

    const promise=(async()=>{
      const endpoint=endpointFor(kind);
      const settled=await Promise.allSettled(A.providers.map(async provider=>{
        const data=await getJSON(join(provider.base,endpoint),{page});
        const items=arrayFrom(data).map(x=>{
          const item=normalize(x,kind);
          if(item) item.providerName=provider.name;
          return item;
        }).filter(Boolean);
        ...
        return {items,source:provider,raw:data,page,current,hasNext,totalPages};
      }));
      ...
      for(let i=0;i<settled.length;i++){
        const r=settled[i];
        if(r.status==='fulfilled') providerResults.push(r.value);
        else errors.push(`${A.providers[i].name}: ${r.reason?.message||r.reason||'error'}`);
      }

      // Proveedor único: PelisPlusHD.
      const merged=[];
      const seen=new Set();
      const keyFor=item=>{
        ...
        // Primero por ID; como respaldo usamos título+año para duplicados entre APIs.
        return id ? `id:${id}` : `title:${title}|year:${year}`;
      };
      for(const result of providerResults){
        for(const item of result.items||[]){
          const k=keyFor(item);
          if(seen.has(k)){
            const existing=merged.find(x=>keyFor(x)===k);
            if(existing){
              // Conservamos datos que uno de los proveedores pueda tener más completos.
              ...
              if(!existing.providerUrl && item.providerUrl) existing.providerUrl=item.providerUrl;
              if((!existing.genres||!existing.genres.length) && item.genres?.length) existing.genres=item.genres;
            }
            continue;
          }
          seen.add(k);
      ...
      const totalPages=Math.max(0,...providerResults.map(r=>Number(r.totalPages)||0));
      const hasNext=providerResults.some(r=>r.hasNext || (r.totalPages && page<r.totalPages));
      return {
        items:merged,
        providers:providerResults,
        source:providerResults.length===1?providerResults[0].source:null,
        raw:providerResults.map(r=>r.raw),
        page,
        current:page,
        hasNext,
        totalPages,
    ...
    promise.finally(()=>setTimeout(()=>{if(pageRequests.get(key)===promise)pageRequests.delete(key)},0));
    return promise;
  }
  ...
  async function search(kind, query){
    ...
    let nextPage=1;
    let totalPages=0;
    const wanted=normText(q);

    while(nextPage<=maxPages && (!totalPages || nextPage<=totalPages)){
      const pages=Array.from({length:batchSize},(_,i)=>nextPage+i)
        .filter(p=>p<=maxPages && (!totalPages || p<=totalPages));
      if(!pages.length)break;
      const batch=await Promise.all(pages.map(page=>list(kind,page).catch(()=>null)));
      let valid=0, ended=false;
      for(const r of batch){
        if(!r?.items?.length)continue;
        valid++;
        if(r.totalPages) totalPages=Math.max(totalPages,Number(r.totalPages)||0);
      ...
      if(results.length || !valid || ended)break;
      nextPage+=batchSize;
    }
    return results;
  }
  ...
  window.NVApi={getJSON,list,search,detail,resolve,resolveEpisode,genres,normalize,findUrls,findEpisodes,providers:A.providers};
})();
